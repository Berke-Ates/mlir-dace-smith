...
insertTransientArray(Location location, translation::Connector connector,
...
LogicalResult translation::collect(TaskletNode &op, ScopeNode &scope) {
  ...
  llvm::StringMap<Connector> outputNameMap;
  llvm::SmallVector<std::string> outputNames;
  for (unsigned i = 0; i < op.getNumResults(); ++i) {
    std::string name = op.getOutputName(i);

    if (!llvm::is_contained(outputNameMap, name)) {
      Connector connector(tasklet, name);
...
LogicalResult translation::collect(StoreOp &op, ScopeNode &scope) {
  ...
  // If all the operands comes from a map op, thus symbol.
  bool dependsPurelyOnMap = true;
